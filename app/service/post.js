...
class Post extends Service {
  ...
  async find(id) {
    const post = await this.ctx.model.Post.findById(id, { // FIXME: 连表查询
      include: [{
        model: this.ctx.model.User,
        as: 'user', // 与model中定义的关联别名一致
  ...
  async update({ id, user_id, updates }) {
    const post = await this.ctx.model.Post.findByIdWithUser(id, user_id); // FIXME: 调用model中定义好的方法
